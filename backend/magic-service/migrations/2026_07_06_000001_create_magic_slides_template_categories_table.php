...
return new class extends Migration {
    ...
    {
        if (Schema::hasTable('magic_slides_template_categories')) {
            return;
        }

        Schema::create('magic_slides_template_categories', static function (Blueprint $table): void {
            $table->bigIncrements('id')->comment('主键 ID');
            $table->string('organization_code', 64)->comment('分类所属组织编码');
            $table->string('code', 64)->comment('分类唯一编码，全局唯一');
            $table->json('name_i18n')->comment('分类名称，多语言，zh_CN/en_US 必填');
    ...
    {
        Schema::dropIfExists('magic_slides_template_categories');
    }
};
