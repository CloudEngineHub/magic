<?php

declare(strict_types=1);
/**
 * Copyright (c) The Magic , Distributed under the software license
 */
return [
    'validate_failed' => '幻灯片模板参数校验失败',
    'only_official_organization_can_manage' => '仅官方组织可以管理幻灯片模板',
    'not_found' => '幻灯片模板不存在',
    'code_generate_failed' => '幻灯片模板编码生成失败',
    'file_url_generate_failed' => '幻灯片模板文件链接生成失败',
    'code_already_exists' => '幻灯片模板编码已存在',
    'category_not_found' => '幻灯片模板分类不存在',
    'category_code_already_exists' => '幻灯片模板分类编码已存在',
    'tag_not_found' => '幻灯片模板标签不存在',
    'tag_code_already_exists' => '幻灯片模板标签编码已存在',
    'tag_structure_invalid' => '幻灯片模板标签结构不合法',
    'tag_group_cannot_bind' => '幻灯片模板不能绑定标签组',
    'code_string' => '模板编码必须是字符串',
    'code_regex' => '模板编码必须以 PPT- 开头，且只能使用字母、数字和中划线',
    'label_required' => '模板名称是必需的',
    'label_array' => '模板名称必须是数组',
    'label_zh_cn_required' => '模板中文名称是必需的',
    'label_zh_cn_max' => '模板中文名称不能超过100个字符',
    'label_en_us_required' => '模板英文名称是必需的',
    'label_en_us_max' => '模板英文名称不能超过100个字符',
    'description_required' => '模板描述是必需的',
    'description_array' => '模板描述必须是数组',
    'description_zh_cn_required' => '模板中文描述是必需的',
    'description_zh_cn_max' => '模板中文描述不能超过1000个字符',
    'description_en_us_required' => '模板英文描述是必需的',
    'description_en_us_max' => '模板英文描述不能超过1000个字符',
    'thumbnail_file_key_required' => '模板封面图文件 key 是必需的',
    'thumbnail_file_url_generate_failed' => '模板封面图文件链接不存在或不可用',
    'template_file_key_required' => '模板 ZIP 文件 key 是必需的',
    'template_file_url_generate_failed' => '模板 ZIP 文件链接不存在或不可用',
    'template_file_must_be_archive' => '模板文件必须是压缩包（zip/rar/7z/tar/gz/tgz/bz2/xz）',
    'file_key_string' => '文件 key 必须是字符串',
    'file_key_max' => '文件 key 不能超过512个字符',
    'preview_image_file_keys_array' => '模板预览多图文件 key 必须是数组',
    'preview_url_url' => '预览链接格式不正确',
    'preview_url_max' => '预览链接不能超过1024个字符',
    'status_in' => '状态必须是0（不可用）或1（可用）',
    'sort_integer' => '排序值必须是整数',
    'base_usage_count_integer' => '基础使用次数必须是整数',
    'base_usage_count_min' => '基础使用次数不能小于0',
    'page_integer' => '页码必须是整数',
    'page_min' => '页码必须大于等于1',
    'page_size_integer' => '每页数量必须是整数',
    'page_size_min' => '每页数量必须大于等于1',
    'page_size_max' => '每页数量不能超过200',
    'keyword_max' => '关键词不能超过100个字符',
    'code_max' => '模板编码不能超过64个字符',
    'category_code_required' => '分类编码是必需的',
    'category_code_string' => '分类编码必须是字符串',
    'category_code_max' => '分类编码不能超过64个字符',
    'category_code_regex' => '分类编码必须以 PPT-CATE- 开头，且后续只能使用小写字母、数字和中划线',
    'category_name_required' => '分类名称是必需的',
    'category_name_array' => '分类名称必须是数组',
    'category_name_zh_cn_required' => '分类中文名称是必需的',
    'category_name_zh_cn_max' => '分类中文名称不能超过100个字符',
    'category_name_en_us_required' => '分类英文名称是必需的',
    'category_name_en_us_max' => '分类英文名称不能超过100个字符',
    'tag_code_required' => '标签编码是必需的',
    'tag_code_string' => '标签编码必须是字符串',
    'tag_code_max' => '标签编码不能超过64个字符',
    'tag_code_regex' => '标签编码只能使用小写字母、数字、中划线和下划线',
    'tag_parent_id_integer' => '标签父级 ID 必须是整数',
    'tag_parent_id_min' => '标签父级 ID 不能小于0',
    'tag_node_type_in' => '标签节点类型必须是 group 或 tag',
    'tag_usage_type_in' => '标签用途必须是 filter、detail 或 operational',
    'tag_name_required' => '标签名称是必需的',
    'tag_name_array' => '标签名称必须是数组',
    'tag_name_zh_cn_required' => '标签中文名称是必需的',
    'tag_name_zh_cn_max' => '标签中文名称不能超过100个字符',
    'tag_name_en_us_required' => '标签英文名称是必需的',
    'tag_name_en_us_max' => '标签英文名称不能超过100个字符',
    'tag_description_array' => '标签描述必须是数组',
    'tag_description_zh_cn_max' => '标签中文描述不能超过500个字符',
    'tag_description_en_us_max' => '标签英文描述不能超过500个字符',
    'tag_aliases_array' => '标签别名必须是数组',
    'tag_is_visible_boolean' => '标签是否展示必须是布尔值',
    'tag_codes_array' => '标签编码列表必须是数组',
    'tag_codes_string' => '标签编码必须是字符串',
    'tag_match_in' => '标签匹配方式必须是 any 或 all',
];
