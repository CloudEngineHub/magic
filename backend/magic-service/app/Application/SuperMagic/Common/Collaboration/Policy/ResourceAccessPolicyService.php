...
namespace App\Application\SuperMagic\Common\Collaboration\Policy;
...
class ResourceAccessPolicyService extends AbstractKernelAppService
{
    ...
    public function assertEditable(
        ...
        $this->assertOperation($authorization, $resourceType, $code, Operation::Edit);
    }
    ...
    public function assertManageable(
        ...
        $this->assertOperation($authorization, $resourceType, $code, Operation::Admin);
    }
     ...
     * 获取当前用户对可读资源的有效操作权限。
     *
     * 有显式协作权限时返回真实 Operation；仅通过 visibility 可读时按 Operation::Read 返回。
     *
     * @param array<string> $resourceCodes
     * @return array<string, Operation>
     */
    public function getReadableResourceOperations(
        Authenticatable|BaseDataIsolation $authorization,
        OperationResourceType $operationResourceType,
        ResourceVisibilityResourceType $visibilityResourceType,
        ?array $resourceCodes = null
    ): array {
        $readableResult = $this->getReadableResourceCodes($authorization, $operationResourceType, $visibilityResourceType, $resourceCodes);

        /** @var array<string, Operation> $operations */
        $operations = $readableResult['operations'] ?? [];
        foreach ($readableResult['all_codes'] ?? [] as $code) {
            if (! isset($operations[$code])) {
                $operations[$code] = Operation::Read;
            }
        }
    ...
    private function assertOperation(
        ...
        $operation = match ($requiredOperation) {
            Operation::Edit => 'edit',
