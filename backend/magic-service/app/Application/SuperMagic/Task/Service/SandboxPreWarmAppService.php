...
class SandboxPreWarmAppService extends AbstractAppService
{
    ...
    public function __construct(
        ...
        protected SandboxKeepAliveDomainService $sandboxKeepAliveDomainService,
    ...
    public function registerKeepAliveTopic(RequestContext $requestContext, array $preWarmResult): void
    {
        $topicId = (int) ($preWarmResult['topic_id'] ?? 0);
        if ($topicId <= 0) {
            ExceptionBuilder::throw(GenericErrorCode::ParameterValidationFailed, 'keep-alive topic_id is invalid');
        }
        ...
        if ($topicEntity->getUserId() !== $userId || $topicEntity->getUserOrganizationCode() !== $organizationCode) {
            ExceptionBuilder::throw(SuperAgentErrorCode::TOPIC_ACCESS_DENIED, 'topic.access_denied');
        }
        ...
        if ($projectEntity->getUserId() !== $userId || $projectEntity->getUserOrganizationCode() !== $organizationCode) {
            ExceptionBuilder::throw(SuperAgentErrorCode::PROJECT_ACCESS_DENIED, 'project.access_denied');
        }
        if ($projectEntity->isHidden()) {
            ExceptionBuilder::throw(GenericErrorCode::ParameterValidationFailed, 'keep-alive only supports non-hidden special projects');
        }

        $workspaceId = $projectEntity->getWorkspaceId();
        if ($workspaceId === null) {
            ExceptionBuilder::throw(GenericErrorCode::ParameterValidationFailed, 'keep-alive project workspace is invalid');
        }

        $workspaceEntity = $this->workspaceDomainService->getWorkspaceDetail($workspaceId);
        if ($workspaceEntity === null || $workspaceEntity->getWorkspaceType() !== WorkspaceType::UserSpecial->value) {
            ExceptionBuilder::throw(GenericErrorCode::ParameterValidationFailed, 'keep-alive only supports special projects');
        }

        $sandboxId = (string) ($preWarmResult['sandbox_id'] ?? $topicEntity->getSandboxId());
        if ($sandboxId === '') {
            $sandboxId = (string) $topicId;
        }

        $this->sandboxKeepAliveDomainService->registerOrEnable(
            $userId,
            $organizationCode,
            $projectEntity->getId(),
            $topicId,
    ...
    public function cancelKeepAliveTopic(RequestContext $requestContext, int $topicId): array
    {
        if ($topicId <= 0) {
            ExceptionBuilder::throw(GenericErrorCode::ParameterValidationFailed, 'keep-alive topic_id is invalid');
        }
        ...
        $disabled = $this->sandboxKeepAliveDomainService->disableByTopic($topicId, 'user canceled keep alive');

        $this->logger->info('特殊项目沙箱保活取消完成', [
            'project_id' => $projectEntity->getId(),
            'topic_id' => $topicId,
            'disabled' => $disabled,
        ]);
