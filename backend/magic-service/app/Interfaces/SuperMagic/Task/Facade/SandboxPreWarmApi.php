...
use App\Application\SuperMagic\Task\Service\SandboxPreWarmAppService;
...
class SandboxPreWarmApi extends AbstractApi
{
    ...
    public function preWarmSandbox(RequestContext $requestContext): array
    {
        ...
        $keepAliveRaw = $this->request->input('keep-alive', $this->request->input('keep_alive', false));
        $keepAlive = filter_var($keepAliveRaw, FILTER_VALIDATE_BOOLEAN);
        ...
        if ($keepAlive && ! empty($workspaceId)) {
            ExceptionBuilder::throw(GenericErrorCode::ParameterValidationFailed, 'keep-alive only supports topic_id or project_id');
        }
        ...
        if ($keepAlive) {
            $this->sandboxPreWarmAppService->registerKeepAliveTopic($requestContext, $result);
        }
    ...
    #[ApiResponse('low_code')]
    public function cancelKeepAlive(RequestContext $requestContext): array
    {
        $requestContext->setUserAuthorization($this->getAuthorization());

        $topicId = (int) $this->request->input('topic_id', 0);
        if ($topicId <= 0) {
            ExceptionBuilder::throw(GenericErrorCode::ParameterMissing, 'topic_id is required');
        }

        $this->logger->info('收到取消沙箱保活请求', [
            'topic_id' => $topicId,
        ]);

        return $this->sandboxPreWarmAppService->cancelKeepAliveTopic($requestContext, $topicId);
    }
}
