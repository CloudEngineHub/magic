 ...
 */
/**
 * ClassMap replacement for volcengine/ve-tos-php-sdk 2.1.12.
 *
 * 本地差异：
 * 将 Content-Length 转为字符串，以兼容 Guzzle 7.11+
 * 和 guzzlehttp/psr7 2.11+。
 *
 * 升级 ve-tos-php-sdk 时必须重新对比该文件。
 */
...
class TosClient
{
    ...
    public function __construct($configOrRegion = 'cn-beijing', $ak = '', $sk = '', $endpoint = '')
    {
        ...
        } elseif ($configOrRegion instanceof ConfigParser) {
            $this->cp = $configOrRegion;
        } elseif (is_string($configOrRegion)) {
        ...
        if (is_callable(__CLASS__ . '::' . $transFn) && is_callable(__CLASS__ . '::' . $parseFn)) {
            ...
            try {
                if ($method === 'GetObjectToFile') {
                    [$request, $filePath, $doMkdir, $bucket, $key] = self::$transFn($input, $this->cp);
                ...
                if ($method === 'PutObjectFromFile' || $method === 'UploadPartFromFile') {
                    $closeBody = true;
                } elseif ($method === 'PutObject' || $method === 'AppendObject' || $method === 'UploadPart') {
                    if (! $request->body) {
    ...
    public function &preSignedURL(PreSignedURLInput $input)
    {
        ...
        if (($ak = $this->cp->getAk()) && ($sk = $this->cp->getSk())) {
            ...
            $signedHeaders = null;
            [$canonicalHeaders, $contentSha256] = self::getCanonicalHeaders($request, $signedHeaders, $signedHeader);
    ...
    {
        [$method, $requestUri, $headers, $body] = $this->prepareRequest($request);
        $options = [
            'headers' => $headers,
            'stream' => $stream,
            'body' => $body,
    ...
    private function &prepareRequest(HttpRequest &$request)
    {
        self::sign(
            $request,
            $this->cp->getHost($request->bucket),
            $this->cp->getAk(),
            $this->cp->getSk(),
            $this->cp->getSecurityToken(),
            $this->cp->getRegion()
        );

        $headers = $request->headers;
        if (isset($headers[Constant::HeaderContentLength])) {
            $headers[Constant::HeaderContentLength] = (string) $headers[Constant::HeaderContentLength];
        }
        ...
        if ($body) {
            ...
            } elseif ($body instanceof StreamInterface && $body->tell() > 0) {
                // fix auto rewind bug in CurlFactory->applyBody
