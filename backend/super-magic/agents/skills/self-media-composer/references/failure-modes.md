...
- **Generating self-media ops data from AI Card**: forbidden. Published-data import, post-publication review, article ops review, localized equivalents, and ops file updates belong to this self-media/IP-operations workflow, not `ai-card-generator`.
...
- **Creating AI press-release flavor content**: forbidden. Load `references/human-writing-style.md` before drafting; remove macro openings, generic value words, mechanical three-part structure, vague authority, and slogan endings.
- **Adding fake human texture instead of real author voice**: forbidden. Do not invent first-person experience, fake customer proof, forced jokes, exaggerated emotion, or casual filler just to make copy sound human.
