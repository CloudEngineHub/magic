...
The schemas below describe the stable Browser intent. Fields may be absent when the selected backend does not support the capability or the event did not occur.
...
When a wait fails, read the complete error. It includes the current page title and URL when available. Read the page before retrying because a redirect, consent screen, CAPTCHA, or other intermediate page may have replaced the expected destination.
...
- `browser_screenshot` always displays a temporary snapshot attachment. With `output_path`, it also saves a reusable file inside the current workspace and returns the normalized path in `result.data["output_path"]`.
- Screenshot output format is inferred from `.webp`, `.jpg`, `.jpeg`, or `.png`; do not pass a separate format. Omit `scale` and `quality` to reuse the adaptive resolution and compression selected for the Tool Detail snapshot. `scale` accepts 0.5-3.0 when a specific saved size is required. WebP/JPEG `quality` accepts 1-100. PNG is lossless and rejects `quality`.
...
- `browser_find_visual` returns one uniquely matched `label`, its `ref`, the current `document_generation`, and visual `evidence`. It fails instead of guessing when zero or multiple valid labels are identified.
...
- `browser_select.value` accepts an exact HTML option value or one unique visible label. A successful result includes the final selected value and label when available.
- Upload paths may be relative to the current workspace or absolute paths inside it. Paths outside the workspace are rejected. A remote Chrome backend may report file upload as unsupported.
- Read the interaction `content` to understand the dispatched action and its observable outcome. Inspect `navigation`, `opened_pages`, `downloads`, `dialogs`, or `snapshot_diff` in `result.data` only when code must branch on those structured facts.
...
browser_read_console(page_id: str, clear: bool = True, limit: int = 100, session_id: str | None = None)
browser_read_network(page_id: str, clear: bool = True, limit: int = 100, session_id: str | None = None)
```

- Use `browser_evaluate` for targeted page inspection or application-specific JavaScript. Its model-readable content includes a bounded preview of the returned value; the complete JSON value remains in `result.data["value"]`. Do not use it to replace Lens, snapshots, or normal interactions.
- Console and network calls return the newest `limit` entries, default 100 and maximum 500. Keep `clear=True` for incremental reads; it clears that page's current buffer after reading. Use `clear=False` only when the same buffer must remain available for another call.
...
- `screenshot`
- `label_to_ref`
- `label`, `ref`, `evidence`, and `document_generation` for a successful visual match
- `console_entries`
- `network_entries`
- `total_count`, `returned_count`, and `error_count` for diagnostic reads
- `pending_count` for network diagnostic reads

Use top-level handle fields for the next call. Read nested objects only when detailed metadata or deterministic processing is needed. Plural results use arrays such as `result.data["pages"]` and `result.data["sessions"]`.

Page objects use `page_id`, including each item returned by `browser_list_pages`. They do not expose the SDK's internal `id` field name.

For large Browser snapshots or diagnostic batches, inspect only the required fields and small samples instead of printing the entire data object.
