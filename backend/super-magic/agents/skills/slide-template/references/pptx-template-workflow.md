...
The generated package uses a lightweight template package structure:
...
Do not place `previews/` in `<template-dir>/`, and do not include `previews/` in `<template-id>-template.zip`.
...
- `files`: `visual_spec`, `theme_css`, `slides_dir`, `images_dir`, and the sibling `package_zip`.
...
- `<template-dir>/visual-spec.md` exists and describes the reusable design system.
