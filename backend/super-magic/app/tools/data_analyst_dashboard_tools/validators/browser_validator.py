...
class BrowserValidator:
    ...
    async def validate(self, project_dir: Path) -> Dict[str, Any]:
        ...
        try:
            # 创建无头浏览器实例
            config = await BrowserConfigAdapter.build_playwright(str(project_dir))
            browser = await create_browser(config)
            
            # 准备dashboard页面URL
            index_html_path = project_dir / "index.html"
            if not await async_exists(index_html_path):
                return {
                    "success": False,
            ...
            # 启动带有监听脚本的临时HTTP服务器
            http_server, port = await self._start_http_server_with_monitor(project_dir, index_html_path)
            ...
            page_id = pages[0].id
            await browser.navigate(page_id, dashboard_url)
            
            # 等待监听结果（20秒超时）
            try:
                monitor_result = await asyncio.wait_for(
                    browser.evaluate(page_id, "window.dashboardMonitorPromise"),
                    timeout=20.0
                )
            ...
            result = monitor_result if isinstance(monitor_result, dict) else {}
            ...
            if http_server:
                try:
                    ...
                    await asyncio.to_thread(http_server.server_close)
                except Exception as e:
    ...
    async def _start_http_server_with_monitor(self, project_dir: Path, index_html_path: Path) -> tuple[HTTPServer, int]:
        """启动带有监听脚本的临时HTTP文件服务
        ...
        # 读取原始HTML内容
        original_html_content = await async_read_text(index_html_path)
