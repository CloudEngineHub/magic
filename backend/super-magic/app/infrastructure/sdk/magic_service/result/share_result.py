...
class ShareResult(AbstractResult):
    ...
    def _parse_data(self) -> None:
        ...
        self.share_url: Optional[str] = self.get("share_url")
        self.created_at: Optional[str] = self.get("created_at")
        self.deleted_at: Optional[str] = self.get("deleted_at")
        self.is_enabled: bool = self.get("is_enabled", True)
        self.is_password_enabled: bool = self.get("is_password_enabled", self.has_password)
        self.project_name: Optional[str] = self.get("project_name")
        self.workspace_id: Optional[str] = self.get("workspace_id")
        self.workspace_name: Optional[str] = self.get("workspace_name")
        self.copy_count: int = self.get("copy_count", 0)
...
class FindSimilarSharesResult:
    ...
    def __init__(self, data: list):
        self.items: List[ShareResult] = [ShareResult(item) for item in (data or [])]
...
class ShareListResult:
    ...
    def __init__(self, data: dict, *, page: int, page_size: int):
        raw_items = data.get("list") if isinstance(data, dict) else []
        self.items: List[ShareResult] = [ShareResult(item) for item in (raw_items or []) if isinstance(item, dict)]
        self.total: int = int(data.get("total", 0)) if isinstance(data, dict) else 0
        self.page = page
