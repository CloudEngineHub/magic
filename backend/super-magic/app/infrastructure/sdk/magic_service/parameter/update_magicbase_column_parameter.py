"""
Update MagicBase column parameter.
"""

from typing import Any, Dict, Optional

from .create_magicbase_column_parameter import MAGICBASE_MYSQL_LIKE_DATA_TYPES
from .magicbase_base_parameter import MagicBaseBaseParameter


class UpdateMagicBaseColumnParameter(MagicBaseBaseParameter):
    """Parameter for PATCH /api/v1/magicbase/projects/{projectId}/tables/{tableId}/columns/{columnId}."""
    ...
    def __init__(
        ...
        self.column_id = str(column_id).strip() if column_id is not None else ""
    ...
    def validate(self) -> None:
        ...
        if not self.column_id.isdigit():
            raise ValueError("column_id must be a numeric string")
        ...
        if self.dynamic_permission is not None and not isinstance(self.dynamic_permission, dict):
            raise ValueError("dynamic_permission must be an object")
