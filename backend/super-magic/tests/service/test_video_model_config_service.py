...
def test_build_video_model_info_includes_compact_input_mode_rules():
    ...
    assert 'default_size=' not in info
    assert 'no_size=' not in info
    assert '<mode name="standard" task="generate" fields="prompt" duration="3|4|5" resolution="720p|1080p|4k" aspect_ratio="16:9|9:16"/>' in info
    assert '<mode name="omni_reference" task="generate" fields="prompt,reference_image_paths,reference_video_paths" max_count="7" duration="3|4|5" resolution="720p|1080p|4k" aspect_ratio="16:9|9:16">' in info
    assert '<rule name="images_only" reference_image_paths="0-7" reference_video_paths="0" duration="3|4|5" resolution="720p|1080p|4k" aspect_ratio="16:9|9:16"/>' in info
    assert '<rule name="image_and_video" reference_image_paths="0-6" reference_video_paths="0-1" duration="3|4|5" resolution="720p|1080p" aspect_ratio="16:9|9:16"/>' in info
    assert '<mode name="video_edit" task="edit" fields="prompt,reference_video_paths" max_count="7" no_duration="true" resolution="720p|1080p" no_aspect_ratio="true">' in info
    assert '<rule name="video_only" reference_video_paths="1" no_aspect_ratio="true" avoid_resolution="4k" no_duration="true" resolution="720p|1080p"/>' in info
...
def test_build_video_model_info_handles_invalid_generation_constraint_types():
    ...
    assert '<mode name="omni_reference" task="generate" fields="prompt,reference_image_paths" duration="5" no_resolution="true" aspect_ratio="16:9">' in info
    assert '<rule name="images_only" reference_image_paths="0-1" duration="5" no_resolution="true" aspect_ratio="16:9"/>' in info
...
async def test_sync_to_horizon_updates_real_horizon_video_model_context(tmp_path):
    ...
    assert 'size=' not in context
    assert '<mode name="standard" task="generate" fields="prompt" duration="5" resolution="720p" aspect_ratio="16:9"/>' in context
