...
interface AppMenuCardProps {
	...
	sourceTypeLabel?: (value?: AppMenu.SourceType) => string
...
function AppMenuCard({
	...
	if (!data) return null
	const isOfficial = data.source_type === AppMenu.SourceTypeMap.official
	const canEdit = isOfficial || data.editable !== false
	...
	return (
		...
		>
			<Flex vertical gap={6}>
				<span>
					{t("appMenu.columns.sourceType")}: {sourceTypeLabel?.(data.source_type) || "-"}
				</span>
				<span>
