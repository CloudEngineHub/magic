...
const DEFAULT_EXTERNAL_RESOURCE_IDLE_MS = 800
const CANVAS_DELAY_MS = 3500
...
const ALLOWED_TRANSITIONS: Record<InterceptorPhase, InterceptorPhase[]> = {
	...
	[INTERCEPTOR_PHASE.REPLAYING]: [INTERCEPTOR_PHASE.CAPTURE_ONLY, INTERCEPTOR_PHASE.RESTORED],
...
export class SandboxReadyController {
	...
	private async replayLoadCallbacks(): Promise<void> {
		...
		if (!this.onloadCalled && this.capturedOnload) {
			try {
				...
				pendingTasks.push(
					this.trackTask(
						Promise.resolve(
							this.capturedOnload.call(this.iframeWindow, fakeLoadEvent),
						).then(() => undefined),
					),
				)
			} catch (error) {
		...
		for (const record of this.domReadyListeners) {
			...
			try {
				record.called = true
				const task = this.trackTask(this.invokeListener(record.original, fakeDomReadyEvent))
				record.pendingTask = task
				pendingTasks.push(task)
			} catch (error) {
				...
				try {
					...
					idleTimer = setTimeout(() => {
						...
						if (latestErrored.length > 0) {
							fail(
								createResourceError("external resource load failed", latestErrored),
							)
							return
						}
	...
	private waitForCanvasDelay(signal?: AbortSignal): Promise<void> {
		const initialCanvases = this.iframeDocument.querySelectorAll("canvas")
		if (initialCanvases.length === 0) return Promise.resolve()

		// 轮询检测 canvas 是否已被绘制（非全空白），最多等 CANVAS_DELAY_MS
		return new Promise<void>((resolve) => {
			...
			const check = () => {
				...
				if (elapsed >= CANVAS_DELAY_MS) {
					signal?.removeEventListener("abort", onAbort)
					resolve()
					return
				}

				// 每次重新查询 canvas（可能有动态创建的）
				const canvases = this.iframeDocument.querySelectorAll("canvas")
				if (canvases.length === 0) {
					timer = setTimeout(check, 300)
					return
				}

				// 检查是否所有 canvas 都已有绘制内容
				let allPainted = true
				for (let i = 0; i < canvases.length; i++) {
					...
					try {
						...
						for (let j = 0; j < data.length; j += 4) {
							...
							if (
								data[j + 3] > 0 &&
								(data[j] < 250 || data[j + 1] < 250 || data[j + 2] < 250)
							) {
								hasContent = true
								break
							}
						...
						// cross-origin canvas, 无法读取像素，只能等时间
						allPainted = false
						break
					}
				}
