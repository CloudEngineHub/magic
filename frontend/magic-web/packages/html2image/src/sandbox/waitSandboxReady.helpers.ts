...
export function createResourceError(message: string, records: ResourceRecord[]): Error {
...
export function createWindowEvent(iframeWindow: Window, type: string): Event {
	const EventCtor = (iframeWindow as Window & { Event?: typeof Event }).Event ?? Event
...
function buildResourceLabel(kind: ResourceKind, url: string, element: Element): string {
