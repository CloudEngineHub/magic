 ...
 */
export function materializePseudoIcons(iDocument: Document, iWindow: Window): IconBackup[] {
	...
	const treeWalker = iDocument.createTreeWalker(iDocument.body, NodeFilter.SHOW_ELEMENT, null)
...
function processListMarker(element: HTMLElement, iWindow: Window): IconBackup | null {
	...
	if (hasListImage) {
		...
		if (imageUrl)
			marker = createImageMarker({
				iWindow,
				imageUrl,
				fontSizePx: parseFloat(liStyle.fontSize) || 16,
			})
	}
...
function resolveTextMarker(input: { element: HTMLElement; listStyleType: string }): string | null {
	const { element, listStyleType } = input
	const index = resolveListItemIndex(element)
	if (index === null) return null

	if (listStyleType === "decimal") return `${index}.`
	if (listStyleType === "decimal-leading-zero") return `${String(index).padStart(2, "0")}.`
	if (listStyleType === "lower-roman") return `${toRoman({ value: index, upper: false })}.`
	if (listStyleType === "upper-roman") return `${toRoman({ value: index, upper: true })}.`
	...
	try {
		...
		if (borderColor !== "transparent" && (sBT > 0 || sBR > 0 || sBB > 0 || sBL > 0)) {
			...
			ctx.strokeRect(
				offset,
				offset,
				canvasWidth - avgBorderWidth,
				canvasHeight - avgBorderWidth,
			)
		}
