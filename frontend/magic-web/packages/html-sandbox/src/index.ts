 ...
 * - Phase 1 (startIframeRuntime): installs registered runtime plugins immediately.
 *   Magic APIs are supplied by the composition root rather than imported here.
...
import { installRegisteredRuntimePlugins } from "./runtime/RuntimePlugin"
import { getParentOrigin } from "./utils/parentOrigin"

// ─── Phase 1: Runtime shell setup (always runs) ────────────────────────────
...
function activateEditorRuntime(scaleRatio: number): void {
	...
	// Also clean up legacy global references that older shell flows may have set
...
function bindRuntimeMessageListener(): void {
	if (typeof window === "undefined") return
	// document.open() can clear listeners bound to the old document/window; remove
	// first so repeated runtime starts rebind exactly one active listener.
...
export function startIframeRuntime(): void {
	bindRuntimeMessageListener()

	// Plugins do not require DOM readiness — run immediately so APIs are available
	// to inline scripts, matching the previous getMagicMethodsScript behavior.
