		...
		return MagicPluginKit.mount(ctx, root, {
			...
			generate: {
				...
				validate: ({ state, helpers }) => {
					...
					if (state.refCropImage && helpers.collectReferenceIds([state.refCropImage]).length !== 1) {
						return t("error.references", "图片缺少可用于生成的资源标识")
					}
...
function buildClothingRepairRequest({ state, helpers, locale, selectedSize }) {
	...
	return {
		...
		prompt: buildClothingRepairPrompt({ repairType: state.repairType, locale, hasCrop, hasRefCrop }),
