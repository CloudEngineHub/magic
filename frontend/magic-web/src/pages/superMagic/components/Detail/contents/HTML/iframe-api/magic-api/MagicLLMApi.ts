...
export class MagicLLMApi extends BaseRuntimeBridgeApiPlugin {
	constructor() {
		super("MagicLLMApi")
	}
		...
		window.Magic.llm = {
			...
			getModels: (options?: { timeout?: number }): Promise<LLMModelInfo[]> => {
				return new Promise<LLMModelInfo[]>((resolve, reject) => {
					...
					this.logger.info("getModels:start", {
						requestId,
					...
					const timer = setTimeout(() => {
						...
						this.logger.error("getModels:timeout", {
							requestId,
						})
					...
					const handler = (
						...
						if (event.data.success) {
							...
							this.logger.error("getModels:failure", {
								requestId,
			...
			chat: (messages: LLMMessage[], options?: LLMOptions): Promise<string> => {
				return new Promise<string>((resolve, reject) => {
					...
					const timer = setTimeout(() => {
						window.removeEventListener("message", handler)
						this.logger.error("chat:timeout", {
							requestId,
						})
					...
					const handler = (
						...
						if (event.data.success) {
							...
							this.logger.error("chat:failure", {
								requestId,
								error: event.data.error,
							})
			...
			stream: (
				...
				const handler = (
					...
					if (event.data.type === "MAGIC_LLM_STREAM_CHUNK") {
						...
						if (event.data.done) {
							this.logger.info("stream:done", { requestId })
							window.removeEventListener("message", handler)
						}
					} else if (event.data.type === "MAGIC_LLM_STREAM_ERROR") {
						this.logger.error("stream:failure", {
							requestId,
							error: event.data.error,
						})
						window.removeEventListener("message", handler)
				...
				return () => {
					if (aborted) return
					aborted = true
					this.logger.warn("stream:abort", { requestId })
					window.removeEventListener("message", handler)
					window.parent.postMessage(
