...
export class MagicAgentApi extends BaseRuntimeBridgeApiPlugin {
	constructor() {
		super("MagicAgentApi")
	}
	...
	private installGetAgents(): void {
		const getAgentsFn = (): Promise<AgentInfo[]> => {
			this.logger.info("getAgents:start")
	...
	private installCreateTopicAndSend(): void {
		const createTopicAndSendFn = (
			...
			if (typeof message === "string") {
				...
				return Promise.reject(
					new Error(
						"createTopicAndSend: message must be a non-empty string or a valid tiptap JSON object",
					),
				)
			}
			this.logger.info("createTopicAndSend:start", {
				message:
					typeof message === "string"
						? MagicApiLogger.summarizeText(message)
						: "[TiptapJSON]",
	...
	private installSendMessage(): void {
		const sendMessageFn = (
			...
			if (typeof message === "string") {
				...
				return Promise.reject(
					new Error(
						"sendMessage: message must be a non-empty string or a valid tiptap JSON object",
					),
				)
			}
			this.logger.info("sendMessage:start", {
				message:
					typeof message === "string"
						? MagicApiLogger.summarizeText(message)
						: "[TiptapJSON]",
