...
const GENERATION_MODE_DEFINITIONS = [
	{
		...
		promptSuffix: {
			...
			en: "Keep the pose change stable, natural, and commercially polished. Avoid exaggerated body distortion while preserving the same face identity and campaign tone. Prefer commercially natural poses over extreme editorial contortion.",
		},
	},
	{
		value: "advanced",
		...
		promptSuffix: {
			...
			en: "Enhance limb transitions, weight balance, hand and foot naturalness, and realistic garment folds for the new pose, while still preserving the same face identity, styling direction, and scene atmosphere. Do not introduce anatomically impossible or surreal body positions.",
		},
	},
]
...
function buildPoseSwapPrompt({ poseMode, posePrompt, generationMode, genCount, locale }) {
		...
		return ctx.panel.render(root, {
			...
			generate: {
				...
				execute: async ({ state, helpers, generateAndPlace }) => {
					...
					const requests = buildImagePoseSwapRequests({
						state,
						helpers,
						locale: promptLocale,
					})
