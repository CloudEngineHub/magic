...
const MANIFEST_TTL_OPTIONS: Record<HtmlPermissionScope, HtmlPermissionTtlOption[]> = {
	...
	"project.message.write": ttlOptions([5, 10, 15]),
...
const LEGACY_TTL_OPTIONS: Record<HtmlPermissionScope, HtmlPermissionTtlOption[]> = {
	"llm.use": ttlOptions([5, 15]),
	"project.message.write": ttlOptions([5, 15]),
	"project.files.upload": ttlOptions([5, 15]),
	"project.files.download": ttlOptions([5, 15]),
	"fs.project.read": ttlOptions([5, 15]),
	"fs.project.write": [
		{ labelKey: "htmlEditor.permissionAuthorizationConfirm.ttl.once", ttlMs: 0 },
		...ttlOptions([5]),
	],
	"user.profile.name": ttlOptions([5, 15]),
	"user.profile.identity": ttlOptions([5, 15]),
	"user.profile.organization": ttlOptions([5, 15]),
}
