...
function PresetThumbnail({ value }: { value: string }) {
	switch (value) {
		case "neo-brutalism":
			...
			return (
				<div className="relative h-full w-full overflow-hidden rounded-md bg-[#FAFAF8]">
					...
					/>
					{/* Red margin line */}
					<div className="absolute left-[14%] top-0 bottom-0 w-[1px] bg-red-400/30" />
		...
		case "code-dispatch":
			...
			return (
				<div className="relative h-full w-full overflow-hidden rounded-md bg-white">
					...
					/>
					{/* Top red bar */}
					<div className="absolute top-0 left-0 right-0 h-[3px] bg-[#DD0000]" />
					{/* Content lines */}
		...
		case "dark-tech":
			...
			return (
				<div className="relative h-full w-full overflow-hidden rounded-md bg-[#0a0a0f]">
					...
					/>
					{/* Gold accent bar */}
					<div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-[#d4b07a] to-[#f0d090]" />
					{/* Content */}
		...
		case "ins-modern":
			...
			return (
				<div className="relative h-full w-full overflow-hidden rounded-md bg-[#fafafa]">
					...
					{/* Content */}
					<div className="relative z-[1] flex flex-col items-center justify-center h-full gap-[3px]">
		...
		case "custom":
			return (
				<div className="flex h-full w-full items-center justify-center rounded-md bg-gradient-to-br from-violet-100 to-pink-100">
		...
		<div>
						...
						<Tooltip key={preset.value}>
							<TooltipTrigger asChild>
								<button
									type="button"
									className={cn(
										"group relative flex items-center gap-2 overflow-hidden rounded-lg border text-left transition-all duration-200",
								...
								>
									...
									{/* Selected indicator */}
									{isSelected && (
										<div className="absolute right-1 top-1 flex h-3 w-3 items-center justify-center rounded-full bg-primary text-white">
											<svg
												width="8"
												height="8"
												viewBox="0 0 24 24"
			...
			{selected === "custom" && onCustomDescriptionChange && (
				<div className={cn(isMd ? "mt-3" : "mt-2")}>
					<div
						className={cn(
							"flex items-center gap-2 w-full rounded-lg border border-input bg-background transition-all",
							"hover:border-border focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20",
							isMd ? "pl-4 pr-2 py-2" : "pl-3 pr-1.5 py-1.5",
						)}
					>
						<input
							type="text"
							className={cn(
								"flex-1 min-w-0 bg-transparent placeholder:text-muted-foreground/60 focus:outline-none",
