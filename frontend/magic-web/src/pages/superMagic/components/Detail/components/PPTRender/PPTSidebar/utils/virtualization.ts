const DEFAULT_DESKTOP_ROW_SIZE = 128
const DESKTOP_HORIZONTAL_INSET = 28
const DESKTOP_VERTICAL_CHROME = 28

interface SlideDimensions {
	width: number
	height: number
}

/**
 * Estimate a desktop row from the live sidebar width and the deck aspect ratio.
 * The fixed inset accounts for viewport/right padding plus the virtual row's horizontal padding.
 */
export function estimateDesktopSlideRowSize(
	sidebarWidth: number,
	{ width, height }: SlideDimensions,
): number {
	if (sidebarWidth <= 0 || width <= 0 || height <= 0) return DEFAULT_DESKTOP_ROW_SIZE

	const thumbnailWidth = Math.max(0, sidebarWidth - DESKTOP_HORIZONTAL_INSET)
	const estimatedSize = Math.ceil((thumbnailWidth * height) / width + DESKTOP_VERTICAL_CHROME)

	return Math.max(DEFAULT_DESKTOP_ROW_SIZE, estimatedSize)
}
