...
function SelfMediaOpsMetricFlipCard({
	...
	return (
		<button
			...
			data-testid={testId}
			className={cn(
				"group/metric relative min-h-[156px] min-w-0 rounded-[22px] text-left [perspective:900px] focus-visible:outline-none focus-visible:ring-[3px] focus-visible:ring-[#18181b]/15",
		...
		>
			...
			>
				<div className="group-hover/metric:bg-white/78 absolute inset-0 overflow-hidden rounded-[22px] border border-white/70 bg-white/55 p-4 shadow-[inset_0_1px_rgba(255,255,255,0.8),0_12px_32px_rgba(47,43,36,0.06)] backdrop-blur transition-[border-color,box-shadow,transform,background] duration-300 [backface-visibility:hidden] group-hover/metric:-translate-y-0.5 group-hover/metric:border-[#ffd637]/70 group-hover/metric:shadow-[inset_0_1px_rgba(255,255,255,0.88),0_18px_42px_rgba(47,43,36,0.1)]">
					<div className="flex items-center gap-2 text-[12px] font-[650] text-[#71717a]">
						{icon}
						<span>{label}</span>
					</div>
					<div
						className={cn("mt-3 truncate text-[34px] font-[820] leading-none", accent)}
					>
						{value}
					</div>
					<div className="self-media-ops-metric-line mt-4 h-1.5 overflow-hidden rounded-full bg-[#18181b]/10">
						<div className="h-full w-2/3 rounded-full bg-[#ffd637] transition-all duration-300 group-hover/metric:w-full" />
					</div>
					<div className="mt-3 flex items-center justify-between gap-2 text-[11px] font-[680] text-[#71717a]">
						<span>{statusLabel}</span>
						<span className="text-[#18181b]">查看拆解</span>
					</div>
				</div>
				<div
					className="absolute inset-0 flex flex-col overflow-hidden rounded-[22px] border border-[#18181b]/10 bg-[#18181b] p-4 text-white shadow-[inset_0_1px_rgba(255,255,255,0.16),0_18px_42px_rgba(24,24,27,0.18)] [backface-visibility:hidden] [transform:rotateY(180deg)]"
					...
					<div className="mt-2 space-y-2">
						{detail.rows.map((row) => (
							<div
								...
								className="flex items-start justify-between gap-3 text-[11px] leading-[1.35]"
							>
								<span className="shrink-0 text-white/55">{row.label}</span>
								<span className="min-w-0 truncate text-right font-[760] text-white">
									{row.value}
								</span>
							</div>
						))}
