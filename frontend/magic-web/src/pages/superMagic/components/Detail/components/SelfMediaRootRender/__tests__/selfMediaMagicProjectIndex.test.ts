...
window.magicProjectConfigure(window.magicProjectConfig);`
	...
	it("sets publish status in a JS object-literal magic.project.js generated by self-media composer", () => {
		const current = `window.magicProjectConfig = {
  version: "1.0.0",
  type: "self-media",
  name: "My Project",
...
window.magicProjectConfigure(window.magicProjectConfig);`
		...
		const updated = setSelfMediaPostPublishStatusInIndex(current, {
			platform: "rednote",
			id: "post-a",
			entry: "posts/post-a/post.json",
			publishStatus: "archived",
		...
		const config = parseConfig(updated)
		expect(config["self-media"].rednote.posts[0]).toEqual({
			id: "post-a",
			name: "Post A",
			entry: "posts/post-a/post.json",
			publishStatus: "archived",
