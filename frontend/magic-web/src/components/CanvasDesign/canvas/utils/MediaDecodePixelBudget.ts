...
export function estimateScaledPixelCost(
	width: number,
	height: number,
	maxEdge?: number,
): number {
...
export class MediaDecodePixelBudgetGate {
	...
	private sortQueue(): void {
		this.queue.sort((a, b) => {
			const priorityDiff = getMediaDecodePriorityRank(a.priority) - getMediaDecodePriorityRank(b.priority)
	...
	private canStart(pixelCost: number): boolean {
		return this.activePixelCostValue === 0 || this.activePixelCostValue + pixelCost <= this.pixelBudget
	}
